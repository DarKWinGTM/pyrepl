...
def get_token(name, repl, key):
    ...
    '''
    #   CF BLOCK    headers = {
    #   CF BLOCK        'Content-Type': 'application/json',
    #   CF BLOCK        'connect.sid': key,
    #   CF BLOCK        'X-Requested-With': 'https://replit.com',
    #   CF BLOCK        'Origin': 'https://replit.com',
    #   CF BLOCK        'User-Agent': 'Mozilla/5.0'
    #   CF BLOCK    }
    #   CF BLOCK    cookies = {
    #   CF BLOCK        'connect.sid': key
    #   CF BLOCK    }
    #   CF BLOCK    r = requests.get(f'https://replit.com/@{name}/{repl}', headers=headers, cookies=cookies)
    #   CF BLOCK    data = json.loads(r.text.split("<script id=\"__NEXT_DATA__\" type=\"application/json\">")[1].split("</script>")[0])['props']['pageProps']['connectionMetadata']
    #   CF BLOCK    return data['token'], f"{data['gurl']}/wsv2/{data['token']}"
    r = cloudscraper.create_scraper(
        delay   = 32
    ).get(
        f'https://replit.com/@{ name }/{ repl }', 
        headers = {
            'Content-Type'      : 'application/json',
            'connect.sid'       : key,
            'X-Requested-With'  : 'https://replit.com',
            'Origin'            : 'https://replit.com',
            'referer'           : f'https://replit.com/@{ name }/{ repl }',
            'Cache-Control'     : 'no-cache',
            'Cookie'            : f'connect.sid={ key }; ',
        },
        cookies = {
            'connect.sid'   : key
        },
